...
bool input_prepareDynamicInput(run_t* run, bool need_mangele) {
    {
        ...
        if (run->dynfileqCurrent == NULL) {
            ...
            run->CurrentqIteration = run->dynfileqCurrent->score;
            // fprintf(file, "SCORE:%d\n", run->CurrentqIteration);
            // fclose(file);
        } 
       else {
            if (run->CurrentqIteration > 0) {
              // run->dynfileqCurrent = TAILQ_FIRST(&run->global->io.dynfileq);
              run->CurrentqIteration--;
              // fprintf(file, "SCORE:%d\n", run->CurrentqIteration);
              // fclose(file);
            }
            else if (run->dynfileqCurrent == TAILQ_LAST(&run->global->io.dynfileq, dyns_t)) {
                run->dynfileqCurrent = TAILQ_FIRST(&run->global->io.dynfileq);
                run->CurrentqIteration = run->dynfileqCurrent->score;
            } 
            else {
